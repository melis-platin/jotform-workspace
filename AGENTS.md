...
- Publish > Push Notifications > Quick Preview notification card styling must stay scoped to the `.push-preview-phone__notification` mockup only. Preserve its native iOS-style system font stack, compact token-based padding/gaps, translucent blurred surface, and title/time/content alignment. The notification title inside `.push-preview-phone__notification-head p` must always be 16px (`var(--font-size-md)`) and use the bold font-weight token. The notification content inside `.push-preview-phone__notification-text` must always be 16px (`var(--font-size-md)`) and use the regular/book font-weight token. Do not apply these styles to notification history cards, permission prompts, or other push notification UI.
