...
- Live preview phone top header action icons must keep stable sizes across states. Search and Push Notifications icons must both be 20px (`var(--space-5)`); never let the notification bell render at 24px or resize these action icons without an explicit design change.
