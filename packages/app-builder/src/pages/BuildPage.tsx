...
export interface AppPage {
  ...
  landing?: boolean
  /** Visibility rules assigned to this page. Rule details are edited separately. */
...
function readChartMeasures(value: unknown, fallbackField: unknown, fallbackAggregation: unknown, numericColumns: ChartSourceColumn[]): BuilderChartMeasure[] {
  if (typeof value === 'string' && value.startsWith('[')) {
    try {
      ...
      // Existing charts use the legacy field/aggregation pair below.
    }
  }
      ...
      {isSelected && (
        <div className="build-page__canvas-element-actions">
          ...
          >
            ...
            <span className="build-page__canvas-element-action-label">Properties</span>
          </button>
          <button
            type="button"
...
export function BuildPage({
  ...
  const [pagePropertiesTab, setPagePropertiesTab] = useState<'general' | 'condition'>('general')
  ...
  const openPageSettings = useCallback((pageId: string, tab: 'general' | 'condition' = 'general') => {
    setActivePageId(pageId)
    setSelectedElementId(null)
    setPagePropertiesId(pageId)
    setPagePropertiesTab(tab)
    setRightPanel('page')
  }, [])
                ...
                {(() => {
                  ...
                  if (isWhatsApp && propertyTab === 'style') {
                    ...
                    return <div className="property-panel__body property-panel__body--whatsapp">
                      ...
                      {displayStyle === 'Button' && <>
                        {renderWhatsAppStyleOptions('Button Alignment', 'Button Alignment', ['Left', 'Center', 'Right'])}
                      ...
                      return (
                        <div className="property-panel__body chart-properties">
                          <section className="chart-properties__section chart-properties__section--source">
                            <DSFormField title="Chart Source" description={<span className="chart-properties__source-description"><span>This chart shows data from App Tables</span><DSLink href="#" size="sm" rightIcon={<Icon name="arrow-up-right-from-square" category="arrows" size={14} />} onClick={(event) => { event.preventDefault(); onOpenDataTableForElement?.(selectedElement.id, pages) }}>Open Data</DSLink></span>} size="md" showDescription showHelpText={false}>
                    ...
                    <button
                      ...
                      onClick={() => {
                        ...
                        const currentSettings = {
                          Measures: chartElement.properties.Measures,
                          'Measure Field': chartElement.properties['Measure Field'],
                          Aggregation: chartElement.properties.Aggregation,
                        ...
                        setPages((currentPages) => currentPages.map((page) => ({
                          ...
                          elements: page.elements.map((element) => element.id === chartElement.id ? {
                            ...
                            properties: {
                              ...
                              'Chart Table Rows': JSON.stringify(sourceRows),
                              'Table Settings': JSON.stringify({ ...savedSettings, [currentSource]: currentSettings }),
                            },
                          } : element),
                        })))
                      }}
